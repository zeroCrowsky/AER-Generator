...
left  = bs.bisect_left
right = lambda xs, x : bs.bisect_right(xs, x) - 1 
...
class TemporalSequenceView(object):
    def __init__(self, data, times, sequence_factory=rangeview_factory, 
            find_start=bs.bisect_left, find_end=bs.bisect_left):
        self.data  = data
        self.times = times
    ...
    def sequence(self, tstart=0, tend=None):
        start = 0                   if tstart == 0    else self.find_start(self.times, tstart)
        end   = len(self.times) - 1 if tend   is None else self.find_end(self.times, tend)
    ...
    def next(self, tduration):
        ...
        # Compute idx
        start = self.time_idx
        end   = self.find_end(self.times, self.time_cur)
